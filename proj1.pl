...
predict_lm(X_hat, DATA_x, DATA_y, ANS, N) :- number(N), remove_row(N, DATA_x, N_DATA_x), \+number(DATA_y), regress_lm(N_DATA_x, DATA_y, [[INTERCEPT]|BETAS]), mmul([X_hat], BETAS, [[ANS_NOBIAS]]), ANS is ANS_NOBIAS + INTERCEPT.
